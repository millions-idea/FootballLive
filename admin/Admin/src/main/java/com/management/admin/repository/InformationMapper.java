...
public interface InformationMapper extends MyMapper<Information> {
    ...
    @Update("update tb_informations set live_id=#{liveId},game_id=#{gameId}" +
            ",content=#{content} where information_id = #{informationId}")
    Integer modifyInformationById(Integer informationId);
    ...
    @Select("SELECT COUNT(t1.user_id) FROM tb_informations t1 LEFT JOIN tb_lives t2 ON t2.live_id = t1.live_id \n" +
            "LEFT JOIN tb_games t3 ON t1.game_id = t3.game_id  "+
            "WHERE ${condition}")
    /**
