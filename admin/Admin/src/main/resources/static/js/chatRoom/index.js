    ...
    initDataTable(route + "/getchatRoomLimit", function (form, table, layer, vipTable, tableIns) {
        ...
        table.on('tool(my-data-table)', function(obj){
            ...
            if(layEvent === 'details'){ //详情
                window.location="/management/chatRoomUser/index?roomId="+data.roomId;
            }
        });
    });
})()
...
function getTableColumns() {
    return [[
        ...
        , {field: 'frequency', title: '发言频率', width: 150}
        , {fixed: 'right',title: '操作', width: 160, align: 'center', templet: function(d){
                var html = "";
                html+='<a name="item-view" class="layui-btn layui-btn layui-btn-xs" lay-event="details">查看用户列表</a>';
                return html;
            }}
    ]];
}
