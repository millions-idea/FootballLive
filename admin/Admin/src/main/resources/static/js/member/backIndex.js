...
(function () {
    ...
    initDataTable(route + "/getuserFeedbackLimit", function (form, table, layer, vipTable, tableIns) {
        ...
        table.on('tool(my-data-table)', function(obj){
            ...
            if(layEvent === 'details'){ //重置密码
                service.edit({
                    backUserId: data.userId
                }, function(html){
                    layer.open({
                        type: 1,
...
function initService(r) {
    return {
        ...
        edit: function (param, callback) {
            $.get(r + "/backDetails", param, function (data) {
                callback(data);
            });
        }
    }
...
function getTableColumns() {
    return [[
        ...
        , {field: 'backTime', title: '拉黑时间', width: 180, sort: true, templet: function (d) {
                return d.addDate == null ? '' : utils.date.timestampConvert(d.addDate);
            }}
        , {field: 'blackRemark', title: '拉黑理由', width: 150}
