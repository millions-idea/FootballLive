...
(function () {
    ...
    // 加载数据表
    initDataTable("/competition/getCompetitionLimit", function (form, table, layer, vipTable, tableIns) {
        ...
        table.on('tool(my-data-table)', function(obj){
            ...
            if (layEvent=='add'){
                ...
                layer.confirm('真的删除吗？', function (index) {
                    $.ajax({
                        url: "/competition/deleteGame?gameId=" +data.gameId,
                        type: "get",
                        processData: false,
                        contentType: false,
                        cache: false,
...
function getTableColumns() {
    return [[
        ...
        , {field: 'gameId', title: '赛事ID', width: 150, sort: true}
        , {field: 'gameName', title: '赛事名称', width: 150}
        , {field: 'gameIcon', title: '赛事LOGO', width: 150, templet: function(d){
                var part = 'data-id="' + d.gameId + '" data-nick="' + d.gameName + '"';
                return '<img ' + part + '  width="27px" class="face" src="' + d.gameIcon + '" />';
            }}
        , {field: 'categoryName', title: '赛事分类', width: 150}
