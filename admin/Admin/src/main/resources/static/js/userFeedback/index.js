...
var route = "/management/userFeedback";
var service;
var tableIndex;
(function () {
    ...
    // 加载数据表
    initDataTable(route + "/getuserFeedbackLimit", function (form, table, layer, vipTable, tableIns) {
...
function initService(r) {
    return {
        ...
        details: function (param, callback) {
            $.get(r + "/details", param, function (data) {
                callback(data);
            });
        }
    }
...
function getTableColumns() {
    return [[
        ...
        , {field: 'feedbackId', title: 'ID', width: 80, sort: true}
        , {field: 'userAvatar', title: '用户头像', width: 150, templet: function(d){
                var part = 'data-id="' + d.feedbackId + '" data-nick="' + d.userNick + '"';
                return '<img ' + part + '  width="27px" class="face" src="' + d.userAvatar + '" /> <span style="color: red">点击图片放大</span>';
            }}
        , {field: 'userNick', title: '用户昵称', width: 150}
        , {field: 'userPhone', title: '用户手机号', width: 150}
        , {field: 'addDate', title: '反馈时间', width: 180, sort: true, templet: function (d) {
        ...
        , {fixed: 'right',title: '操作', width: 560, align: 'center', templet: function(d){
                var html = "";
                html += '<a name="item-edit" class="layui-btn layui-btn layui-btn-xs" lay-event="details">详情</a>';
                return html;
            }}
    ]];
}
