...
var historyService = {
	getLiveCollectList: function(param, callback){
		$.get(app.utils.toUrl(app.config.apiUrl + "api/profile/getLiveCollectList"), function(data){
			app.logger("collect", JSON.stringify(data));
			callback(data);
		});
	},
	
	cleanCollect: function(param, callback){
		$.get(app.utils.toUrl(app.config.apiUrl + "api/profile/cleanCollect"), function(data){
			app.logger("collect", JSON.stringify(data));
			callback(data);
		});
	}
...
mui.plusReady(function(){
	...
	$(".clean").click(function(){
		mui.confirm("您确定要清空所有记录吗", "清空历史记录", ["确定","取消"], function(e){
			//确定
			if(e.index == 0){
				historyService.cleanCollect({}, function(res){
	...
	//获取观看历史记录
	historyService.getLiveCollectList({}, function(res){
		if(app.utils.ajax.isError(res)) {
			if(res.msg == "空的集合"){
				return app.utils.msgBox.msg("您还没有收藏过直播哦");
			}
			return app.utils.msgBox.msg("加载个人收藏记录失败");
		}
